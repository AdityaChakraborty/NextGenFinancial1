...
The core planner is fully local and uses `city_goal_costs.csv` as its planning reference. The profile salary model uses `salary_data.csv`. No Supabase, paid API, or RAG is required; model training is optional because the prediction is comparison-only.

## Profile salary model

The planner uses the primary profile inputs City, Education, and Job Role with the local salary model to estimate monthly salary. The entered real salary is always used to calculate goals; the prediction is shown as a comparison:

```bash
python train_model.py --data salary_data.csv --artifact artifacts/salary_predictor.joblib
```

The training pipeline cleans the supplied data, removes the empty trailing column and `Age`, compares Gradient Boosting, Random Forest, Extra Trees, and an MLP neural-network baseline, and saves the model with the lowest cross-validated mean absolute error. Check `/api/v1/ml/status` or call `/api/v1/ml/predict-salary` with `City`, `Education`, and `Job_Role` after training. The prediction and entered salary are both returned in `calculation_profile`; the entered salary controls affordability.
...
Each investment option uses an equal share of one portfolio contribution slider. Changing the total recalculates each category's projected future value, the summed tenure value, and the selected portfolio total. Name, age, experience level, city, education, and job role are primary profile inputs; age shapes the recommendation horizon, city shapes goal costs, and education/job role shape the salary prediction. Experience level supports `0-1` Fresher, `1-3` Experienced, and `3-5` Expert; it is stored with the submitted profile, while the salary dataset does not yet contain an experience feature.
